...
public class User {
	...
	public enum Modes
	{
		NO_CTCP('C', Type.PREVENT_MESSAGES), DEAF('D', Type.PREVENT_MESSAGES), CALLER_ID('g', Type.PREVENT_MESSAGES), SOFT_CALLER_ID(
				'G', Type.PREVENT_MESSAGES), REG_ONLY('R', Type.PREVENT_MESSAGES), INVISIBLE('i', Type.ADD_PRIVACY), PRIVATE(
				'p', Type.ADD_PRIVACY), CLOAKED('x', Type.ADD_PRIVACY), REGISTERED('r', Type.SERVICE_ONLY), SSL('S',
				Type.SERVICE_ONLY), CGI_IRC('W', Type.SERVICE_ONLY);

		private final char _mode;
		private final Type _type;

		private Modes(char c, Type type) {
			_mode = c;
			_type = type;
		}
		...
		public enum Type
		{
			PREVENT_MESSAGES, ADD_PRIVACY, SERVICE_ONLY;
		}
	}
