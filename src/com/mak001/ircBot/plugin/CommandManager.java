...
public class CommandManager {
	...
	public boolean addPluginCommand(Plugin plugin) {
		System.out.println("Adding plugin command " + plugin.GENERAL_COMMAND + " for " + plugin.getName());
		if (commands_by_plugin_name.get(plugin.getName().toUpperCase()) == null) commands_by_plugin_name.put(plugin
				.getName().toUpperCase(), new ArrayList<Command>());
	...
	public boolean addCommand(Command command) {
		System.out.println("Adding command " + command.getCommand()[0] + "; " + getPluginNameByCommand(command));
		if (full_command_list.contains(command)) return false;
		if (commands_by_plugin_name.get(getPluginNameByCommand(command)) == null) {
			commands_by_plugin_name.put(getPluginNameByCommand(command), new ArrayList<Command>());
		}
		commands_by_plugin_name.get(getPluginNameByCommand(command)).add(command);
	...
	public Command getCommand(String command) {
		...
		for (Command c : full_command_list) {
			for (String s : c.getCommand())
				if (command.startsWith(s.toUpperCase())) return c;
		}
		return null;
	}
	...
	public void removeCommand(Command command) {
		if (full_command_list.contains(command)) {
			...
			commands_by_plugin_name.get(getPluginNameByCommand(command)).remove(command);
		}
	}
	...
	public void removeCommands(Plugin plugin) {
		for (Command command : commands_by_plugin_name.get(plugin.getName().toUpperCase())) {
